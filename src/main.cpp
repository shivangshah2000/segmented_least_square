...
int main() {
    ...
    while(true) {
        char ch;
        cout << "Enter points in the file input.txt - enter 'q' to continue" << endl;
        cin >> ch;
        if(ch == 'q'){break;}
    }
        ...
        for(i=1;i<=j;i++){
            ...
            if(denominator == 0){
                slope[i][j] = INT_MAX;            // infinity value
                    ...
                    slope[i][j] = ((j- i +1 )*xy_seg_sum - x_seg_sum*y_seg_sum) / (double(denominator));
            }
            intercept[i][j] = ( y_seg_sum - slope[i][j]*x_seg_sum ) / double((j- i +1 )) ;
            for(k=i;k<=j;k++){
                double error = points[k].second - ( slope[i][j] *points[k].first + intercept[i][j] ) ;        //  y - yi  where yi = ax +b
                E[i][j] += error*error;
            }
        }
    }
	...
	for (i = N, j = optimal_segment[N]; i > 0; i = j-1, j = optimal_segment[i])	{
		temp_stack.push(i);
		temp_stack.push(j);
	}
	int seg_count = 0;
	...
	while (!temp_stack.empty())	{
        ...
        string s = to_string(points[i].first) + " " + to_string(points[i].second) + ":" + to_string(points[j].first) + " " + to_string(points[j].second);
        outfile << s << endl;
        cout<<"The line segment : y = "<<slope[i][j]<<"x + "<<intercept[i][j] << " will cover points between "<<'('<<
        points[i].first<<','<<points[i].second<<')'<<" to "<<'('<<points[j].first<<','<<points[j].second<<')'<<endl;
        cout<<"Error associated with above line segment is : "<<E[i][j]<<endl<<endl;
	}
    ...
    outfile<< "#"<<endl;
    outfile<< OPT[N] <<endl;              // error value 
